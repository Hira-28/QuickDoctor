...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetDoctorsBySpecialization` (IN `spec_name` VARCHAR(100))   BEGIN
    SELECT id, name, room_no
    FROM doctor
    WHERE specialist = spec_name;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetNursesByDepartment` (IN `dept_name` VARCHAR(100))   BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `before_delete_doctor` BEFORE DELETE ON `doctors` FOR EACH ROW BEGIN
    -- Log the deletion attempt
    INSERT INTO doctor_logs (action, doctor_id, name, timestamp)
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `before_insert_doctor` BEFORE INSERT ON `doctors` FOR EACH ROW BEGIN
    -- Log the insertion attempt
    INSERT INTO doctor_logs (action, doctor_id, name, timestamp)
    VALUES ('INSERT', NEW.id, NEW.name, NOW());

    -- Validate specialization
    IF NEW.specialist IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Specialization cannot be NULL';
    END IF;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `before_delete_employee` BEFORE DELETE ON `employee` FOR EACH ROW BEGIN
    -- Log the deletion attempt
    INSERT INTO employee_logs (action, employee_id, name, timestamp)
    VALUES ('DELETE', OLD.id, OLD.name, NOW());
END
$$
DELIMITER ;
...
(1742, 'rahim', 'Dhaka', 'pathology');

--
-- Triggers `nurse`
--
DELIMITER $$
CREATE TRIGGER `before_delete_nurse` BEFORE DELETE ON `nurse` FOR EACH ROW BEGIN
    -- Log the deletion attempt
    INSERT INTO nurse_logs (action, nurse_id, name, timestamp)
    VALUES ('DELETE', OLD.id, OLD.name, NOW());

    -- Example: Restrict deletion if department is "Critical Care"
    IF OLD.department = 'Critical Care' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot delete nurses from Critical Care department';
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `before_insert_nurse` BEFORE INSERT ON `nurse` FOR EACH ROW BEGIN
    -- Example: Log the insertion attempt
    INSERT INTO nurse_logs (action, nurse_id, name, timestamp)
    VALUES ('INSERT', NEW.id, NEW.name, NOW());

    -- Example: Validation (ensure department is not NULL)
    IF NEW.department IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Department cannot be NULL';
    END IF;
END
$$
DELIMITER ;
...
--
-- Table structure for table `nurse_logs`
--

CREATE TABLE `nurse_logs` (
  `log_id` int(11) NOT NULL,
  `action` varchar(10) NOT NULL,
...
--
-- Dumping data for table `nurse_logs`
--

INSERT INTO `nurse_logs` (`log_id`, `action`, `nurse_id`, `name`, `timestamp`) VALUES
(1, 'INSERT', 1742, 'rahim', '2024-12-11 09:05:06'),
(2, 'DELETE', 5, 'Nasrin Jahan', '2024-12-11 09:13:03');

-- --------------------------------------------------------

--
-- Stand-in structure for view `nurse_view`
-- (See below for the actual view)
--
CREATE TABLE `nurse_view` (
`id` int(11)
,`name` varchar(100)
,`department` varchar(100)
,`address` varchar(255)
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `doctor_view`  AS SELECT `doctors`.`id` AS `id`, `doctors`.`name` AS `name`, `doctors`.`specialist` AS `specialist`, `doctors`.`room_no` AS `room_no` FROM `doctors` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `employee_view`  AS SELECT `employee`.`id` AS `id`, `employee`.`name` AS `name`, `employee`.`department` AS `department` FROM `employee` ;

-- --------------------------------------------------------

--
-- Structure for view `nurse_view`
--
DROP TABLE IF EXISTS `nurse_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `nurse_view`  AS SELECT `nurse`.`id` AS `id`, `nurse`.`name` AS `name`, `nurse`.`department` AS `department`, `nurse`.`address` AS `address` FROM `nurse` ;
...
--
-- Indexes for table `nurse_logs`
--
ALTER TABLE `nurse_logs`
  ADD PRIMARY KEY (`log_id`);
...
--
-- AUTO_INCREMENT for table `nurse_logs`
--
ALTER TABLE `nurse_logs`
